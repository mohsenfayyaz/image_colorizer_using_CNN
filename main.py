...
def get_train(use_saved_npy=True):
    ...
    if use_saved_npy:
        ...
        # gc.collect()
        # train_x = tf.keras.applications.inception_v3.preprocess_input(train_x)
        gc.collect()
...
def get_model(use_saved=True, type="inception"):
    if use_saved:
        model = tf.keras.models.load_model(MODEL_FOLDER + 'model_' + str(LOAD_MODEL_OFFSET) + '.h5')
        return model
    else:
        if type == "inception":
            model = inception_model()
            # model.load_weights(MODEL_FOLDER + 'model_' + str(180) + '.h5')
            return model
        else:
            return cnn_model()
...
class CustomSaver(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs={}):
        if epoch % SAVING_MODEL_EPOCH_INTERVAL == 0:  # or save after some epoch, each k-th epoch etc.
            ...
            show_predict(self.model, global_train_x[6000], epoch + offset)
...
def fit_on_inception(train_x, train_y):
    ...
    saver = CustomSaver()
    es = tf.keras.callbacks.EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=2500)
    model.fit(x=inception_train_x, y=train_y, validation_split=0.04, batch_size=20, epochs=10000, callbacks=[saver, es])
