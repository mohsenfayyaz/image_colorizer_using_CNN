...
class DataGenerator:
    ...
    def get_train(self, ):
        ...
        t = time.time()
        train_x = np.asarray([img_to_np_array(resize(PIL.Image.open(str(image_url)))) for image_url in input_images_url])
        # train_x = np.array([np.array(PIL.Image.open(image_url)) for image_url in input_images_url])
        print("train_x ready")
        train_y = np.asarray([img_to_np_array(resize(PIL.Image.open(str(image_url)))) for image_url in output_images_url])
